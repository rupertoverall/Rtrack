...
calculate_mwm_metrics = function(path, arena){
	...
	metrics = list(
		...
		summary = c(
			...
			distance.from.goal = d.goal.quantile[3] * arena$correction$r,
			distance.from.old.goal = d.old.goal.quantile[3] * arena$correction$r,
			initial.heading.error = initial.heading.error$median,
			initial.trajectory.error = initial.trajectory.error.error * arena$correction$r,
			initial.reversal.error = initial.reversal.error.error * arena$correction$r,
			efficiency = efficiency,
			roaming.entropy = roaming.entropy,
			...
			time.in.far.wall.zone = time.in.zone$far.wall * total.time * arena$correction$t,
